...
int getEvenWordCount(SentStruct *sent)
{
	int evenWordCount = 0;
	int len = 0;
	...
	for (int i = 0; i < sent->length; i++)
	{
		...
		{
			if (len % 2 == 0 && len)
				/* если длина четная и ненулевая (до встречи разделителя было слово) */
			{
				evenWordCount++;
			}
			len = 0;
...
{
	SentStruct *firstSent = (SentStruct *) first;
	SentStruct *secondSent = (SentStruct *) second;

	return secondSent->evenWordCount - firstSent->evenWordCount;
}

void sortSentencesByEvenWordCount(TextStruct *text)
{
	int totalEvenWordCount = 0;

	for (int i = 0; i < text->sentenceCount; i++)
	{
		text->sentenceArray[i].evenWordCount = getEvenWordCount(&text->sentenceArray[i]);
		totalEvenWordCount += text->sentenceArray[i].evenWordCount;
	}
	if (totalEvenWordCount)
		/* если хотя бы в одном предложении есть слово с чётной длиной */
	{
		qsort(text->sentenceArray, text->sentenceCount, sizeof(SentStruct), cmp);
	}
}